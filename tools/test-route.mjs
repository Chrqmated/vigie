...
for (const p of plans) if (p.oppositeCount) console.log(`  [${p.label}] radars à contresens écartés : ${p.oppositeCount}`);
ok(plans.some(p => p.oppositeCount > 0), 'filtre du sens : au moins un radar à contresens écarté sur l’A1');
const fuel = await import('../js/fuel.js');
const st = await fuel.stationsAlong(plans[0].coords, plans[0].cum);
console.log('  stations :', st.map(s => `${s.city} ${s.price} €/L (détour ${s.detourMin} min)`).join(' | '));
ok(st.length > 0 && st.every(s => s.off <= 2000), 'stations les moins chères à ≤ 2 km du tracé');
const viaPlans = await route.plan(from, dest, { via: [{ lat: 49.9310, lon: 2.9320, name: 'Péronne' }] });
ok(viaPlans[0].coords.some(c => Math.hypot(c[0] - 49.931, c[1] - 2.932) < 0.01), 'étape intermédiaire respectée (passe par Péronne)');
